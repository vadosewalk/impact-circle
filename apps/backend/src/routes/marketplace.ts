...
import { db, tenders, drives, categories, ngo, comments, driveUpdates, user } from "@impact/db";
...
marketplaceRoutes.post("/tenders/:id/fulfill", requireAuth, async (c) => {
	...
	if (!tenderRecord) return c.json({ message: "Tender not found" }, 404);
	if (tenderRecord.claimedById !== currentUser.id) return c.json({ message: "Only the claimant can fulfill this tender" }, 403);
...
marketplaceRoutes.post("/drives/:id/update", requireAuth, requireRole("ngo"), async (c) => {
	...
	// TRUST SCORE: +5 for posting impact proof
	await db.update(user)
...
marketplaceRoutes.post("/drives/:id/complete", requireAuth, requireRole("ngo"), async (c) => {
	...
	// TRUST SCORE: +20 for completing a drive
	await db.update(user)
		.set({ trustScore: sql`${user.trustScore} + 20` })
		.where(eq(user.id, currentUser.id));

	return c.json({ message: "Drive marked as completed. +20 Trust Score!" });
});
