...
export default function NgoDashboard() {
	...
	const handlePostUpdate = async (driveId: string) => {
		...
		try {
			...
			toast.success(res.message || "Impact update posted!");
			setUpdateContent("");
		} catch (err: any) {
	...
	const handleCompleteDrive = async (driveId: string) => {
		try {
			const res: any = await api.post(`/api/marketplace/drives/${driveId}/complete`, {});
			toast.success(res.message || "Drive marked as completed!");
			fetchNgoData();
		} catch (err: any) {
	...
	return (
		<div className="max-w-6xl mx-auto px-4 py-8">
							...
							<FieldGroup>
								...
								<div className="grid grid-cols-2 gap-4">
									<Field>
										...
										<Input id="funds" type="number" value={targetFunds} onChange={(e) => setTargetFunds(e.target.value)} />
									</Field>
									<Field>
										<FieldLabel htmlFor="volunteers">Target Volunteers</FieldLabel>
										<Input id="volunteers" type="number" value={targetVolunteers} onChange={(e) => setTargetVolunteers(e.target.value)} />
									</Field>
								</div>
							</FieldGroup>
							<div className="flex justify-end pt-4">
								<Button type="submit" disabled={isCreating}>{isCreating ? "Creating..." : "Launch Drive"}</Button>
							</div>
						</form>
					</DialogContent>
				</Dialog>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold">Active Drives</CardDescription><CardTitle className="text-3xl">{myDrives.length}</CardTitle></CardHeader></Card>
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold">Total Trust Score</CardDescription><CardTitle className="text-3xl">{(session?.user as any)?.trustScore || 0}</CardTitle></CardHeader></Card>
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold">Flag Alerts</CardDescription><CardTitle className="text-3xl text-destructive">{ngoInfo?.flags || 0}</CardTitle></CardHeader></Card>
			</div>
						...
						<Card key={drive.id} className="flex flex-col">
							<CardHeader>
								<div className="flex justify-between items-start mb-2">
									<Badge variant={drive.status === "completed" ? "secondary" : "outline"}>{drive.status.toUpperCase()}</Badge>
							...
							<CardContent className="flex-1">
								...
								<div className="flex gap-6">
									{drive.targetFunds && <div className="flex items-center gap-2 text-sm"><Wallet className="size-4 text-primary" /><span>Target: <b>₹{drive.targetFunds}</b></span></div>}
									{drive.targetVolunteers && <div className="flex items-center gap-2 text-sm"><Users className="size-4 text-primary" /><span>Target: <b>{drive.targetVolunteers}</b></span></div>}
								</div>
							</CardContent>
							<CardFooter className="gap-2 border-t pt-4">
								<Dialog>
									...
									<DialogContent>
										<DialogHeader><DialogTitle>Post Drive Update</DialogTitle></DialogHeader>
										<div className="space-y-4 py-4">
											<p className="text-xs text-muted-foreground">Share photos, receipts, or progress with your donors and volunteers.</p>
											<Textarea 
											...
											<div className="p-4 border-2 border-dashed rounded-lg text-center cursor-pointer hover:bg-muted/50">
												...
												<p className="text-xs text-muted-foreground font-medium">Upload Photos or Receipts</p>
											</div>
										</div>
										<DialogFooter>
											<Button disabled={isPostingUpdate} onClick={() => handlePostUpdate(drive.id)}>Post to Impact Wall</Button>
										</DialogFooter>
									</DialogContent>
								</Dialog>
