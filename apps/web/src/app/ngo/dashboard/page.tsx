...
export default function NgoDashboard() {
	...
	const handleCreateDrive = async (e: React.FormEvent) => {
		...
		try {
			...
			setTitle(""); setDescription(""); setTargetFunds(""); setTargetVolunteers("");
			fetchNgoData();
		} catch (err: any) {
	...
	return (
		<div className="max-w-6xl mx-auto px-4 py-8">
			<header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
				...
				<div className="flex gap-4">
					<Dialog>
						...
						<DialogContent>
							<DialogHeader><DialogTitle>Custom Category Request</DialogTitle></DialogHeader>
							<form onSubmit={handleRequestCategory} className="space-y-4 py-4">
								<FieldGroup>
									<Field>
										<FieldLabel>Tag Name</FieldLabel>
										<Input placeholder="e.g. Animal Medical Rescue" value={catName} onChange={(e) => setCatName(e.target.value)} required />
									</Field>
									<Field>
										<FieldLabel>Purpose</FieldLabel>
										<Textarea placeholder="Explain why this tag is needed for the community..." value={catDesc} onChange={(e) => setCatDesc(e.target.value)} required />
									</Field>
								</FieldGroup>
								<DialogFooter>
									<Button type="submit" disabled={isRequestingCat}>Submit Request</Button>
								</DialogFooter>
							</form>
						</DialogContent>
					</Dialog>
					...
					<Dialog>
						...
						<DialogContent className="max-w-2xl">
							<DialogHeader><DialogTitle>Create a New Drive</DialogTitle></DialogHeader>
							<form onSubmit={handleCreateDrive} className="space-y-6 mt-4">
								<FieldGroup>
									<Field><FieldLabel>Drive Title</FieldLabel><Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} required /></Field>
									<Field><FieldLabel>Detailed Description</FieldLabel><Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} required /></Field>
									<div className="grid grid-cols-2 gap-4">
										<Field><FieldLabel>Target Funds</FieldLabel><Input id="funds" type="number" value={targetFunds} onChange={(e) => setTargetFunds(e.target.value)} /></Field>
										<Field><FieldLabel>Target Volunteers</FieldLabel><Input id="volunteers" type="number" value={targetVolunteers} onChange={(e) => setTargetVolunteers(e.target.value)} /></Field>
									</div>
								</FieldGroup>
								<DialogFooter><Button type="submit">Launch Drive</Button></DialogFooter>
							</form>
						</DialogContent>
					</Dialog>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold tracking-wider">Active Drives</CardDescription><CardTitle className="text-3xl">{myDrives.length}</CardTitle></CardHeader></Card>
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold tracking-wider">Total Trust Score</CardDescription><CardTitle className="text-3xl">{(session?.user as any)?.trustScore || 0}</CardTitle></CardHeader></Card>
				<Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase font-bold tracking-wider">Flag Alerts</CardDescription><CardTitle className="text-3xl text-destructive">{ngoInfo?.flags || 0}</CardTitle></CardHeader></Card>
			</div>
						...
						<Card key={drive.id}>
							<CardHeader>
								<div className="flex justify-between items-start">
									...
									<span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="size-3" /> {new Date(drive.createdAt).toLocaleDateString()}</span>
								</div>
							...
							<CardFooter className="gap-2 border-t pt-4">
								...
								<Button variant="outline" size="sm" className="flex-1">Complete Drive</Button>
							</CardFooter>
						</Card>
					))}
				</div>
