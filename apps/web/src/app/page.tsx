        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b pb-4">
          <TabsList className="grid w-full md:w-[600px] grid-cols-3">
            ...
            <TabsTrigger value="polls">
              Governance{" "}
              {polls.length > 0 && (
                <Badge className="ml-2 py-0 px-1 bg-primary text-primary-foreground">{polls.length}</Badge>
              )}
            </TabsTrigger>
          </TabsList>
          <div className="flex gap-4 w-full md:w-auto">
            <Link href="/tenders/create" className="flex-1 md:flex-none">
              <Button variant="outline" className="w-full">
                <PlusCircle className="size-4 mr-2" /> Post Need
              </Button>
            </Link>
            <Link href="/onboard" className="flex-1 md:flex-none">
              <Button className="w-full">
                <LayoutDashboard className="size-4 mr-2" /> NGO Portal
              </Button>
            </Link>
          </div>
        </div>
                ...
                <Card key={tender.id} className="flex flex-col h-full hover:shadow-md transition-shadow">
                  <CardHeader>
                    ...
                    <CardDescription className="flex items-center gap-1">
                      <MapPin className="size-3" /> {tender.latitude ? "Localized Request" : "Pan-India"}
                    </CardDescription>
                  </CardHeader>
                    ...
                    {tender.targetAmount && (
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs font-medium">
                          <span>Funds: ₹{tender.currentAmount}</span>
                          <span>Target: ₹{tender.targetAmount}</span>
                        </div>
                  ...
                  <CardFooter className="pt-4 border-t flex gap-2">
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Partial Fulfillment Pledge</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                          <p className="text-xs text-muted-foreground">
                            Resource pooling allows multiple users to fulfill a large community need together.
                          </p>
                        ...
                        <DialogFooter>
                          <Button disabled={isSubmitting} onClick={() => handlePledge(tender.id)}>
                            Confirm Pledge
                          </Button>
                        </DialogFooter>
                      </DialogContent>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Public Comment</DialogTitle>
                        </DialogHeader>
                        <textarea
                          className="w-full min-h-[100px] p-2 border rounded-md bg-background text-sm mt-4"
              ...
              <Card key={poll.id} className="border-2 border-primary/20">
                <CardHeader>
                  <div className="flex justify-between items-center">
                    ...
                    >
                      <Vote className="size-3" /> GOVERNANCE POLL
                    </Badge>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex h-3 w-full rounded-full overflow-hidden bg-muted">
                      ...
                      <div
                        ...
                        style={{ width: `${(poll.votesAgainst / (poll.votesFor + poll.votesAgainst || 1)) * 100}%` }}
                      />
                    </div>
                  </div>
                </CardContent>
            ...
            {polls.length === 0 && (
              <div className="col-span-full text-center py-24 bg-muted/30 rounded-lg border-2 border-dashed">
                ...
                <p className="text-muted-foreground">
                  No active governance polls. NGOs can request custom tags from their dashboard.
                </p>
              </div>
            )}
          </div>
