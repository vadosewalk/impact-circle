...
import { MapPin, Clock, MessageSquare, LayoutDashboard, MessageCircle, PlusCircle, Send, CheckCircle2 } from "lucide-react";
...
export default function HomePage() {
	...
	const handlePostComment = async (id: string, type: "tender" | "drive") => {
		...
		try {
			const endpoint = type === "tender" 
				? `/api/marketplace/tenders/${id}/comment`
				: `/api/marketplace/drives/${id}/comment`;
	...
	const handleSendMessage = async (receiverId: string) => {
		...
		setIsSubmitting(true);
		try {
			const res: any = await api.post("/api/messages/send", { receiverId, content: text });
			toast.success(res.message || "Handshake message sent!");
			setText("");
		} catch (err: any) {
	...
	const handleClaimTender = async (tenderId: string) => {
		setIsSubmitting(true);
		try {
			const res: any = await api.post(`/api/marketplace/tenders/${tenderId}/claim`, {});
			toast.success(res.message || "Handshake accepted! You have claimed this tender.");
								...
								<Card key={tender.id} className="flex flex-col h-full hover:shadow-md transition-shadow">
									...
									<CardFooter className="pt-4 border-t flex flex-wrap gap-2">
										<Dialog>
											...
											<DialogContent>
												<DialogHeader><DialogTitle>Public Clarification</DialogTitle></DialogHeader>
										...
										<Dialog>
											...
											<DialogContent>
												<DialogHeader><DialogTitle>Handshake Protocol</DialogTitle></DialogHeader>
												<div className="space-y-4 py-4">
													...
													<div className="space-y-2">
														...
														<p className="text-xs text-muted-foreground">
															Mark this tender as "Claimed" to let the community know it's being handled. 
															You'll be responsible for marking it as fulfilled later.
														</p>
														<Button className="w-full" variant="secondary" disabled={isSubmitting} onClick={() => handleClaimTender(tender.id)}>
								...
								<Card key={drive.id} className="flex flex-col h-full border-primary/20">
									...
									<CardContent className="flex-1">
										...
										<div className="space-y-1">
											{drive.targetFunds && <p className="text-xs flex justify-between"><span>Target:</span> <b>₹{drive.targetFunds}</b></p>}
											{drive.targetVolunteers && <p className="text-xs flex justify-between"><span>Volunteers:</span> <b>{drive.targetVolunteers}</b></p>}
										</div>
									</CardContent>
									<CardFooter className="pt-4 border-t gap-2">
										<Button className="flex-1" size="sm">Pledge Support</Button>
										<Dialog>
											<DialogTrigger render={<Button size="icon" variant="outline" />}>
												<MessageSquare className="size-4" />
											</DialogTrigger>
											<DialogContent>
												<DialogHeader><DialogTitle>Public Drive Comment</DialogTitle></DialogHeader>
												<div className="space-y-4 py-4">
													<textarea 
														className="w-full min-h-[80px] p-2 border rounded-md bg-background text-sm"
												...
												<DialogFooter>
													<Button disabled={isSubmitting} onClick={() => handlePostComment(drive.id, "drive")}>Post Comment</Button>
												</DialogFooter>
											</DialogContent>
										</Dialog>
									</CardFooter>
